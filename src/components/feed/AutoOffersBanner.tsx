...
export default function AutoOffersBanner() {
  ...
  const [offers, setOffers] = useState<AutoOffer[]>([]);
  ...
  const loadOffers = async () => {
    try {
      ...
      if (data?.offers) {
        setOffers(data.offers as AutoOffer[]);
      }
    } catch (error) {
      console.error("Auto offers error:", error);
    }
  };
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
        {offers.map((offer, index) => (
          <button
            type="button"
            key={`${offer.professional_id}-${index}`}
            onClick={() => navigate(`/stylist/${offer.professional_id}`)}
            aria-label={`Apri offerta ${offer.service_name}`}
          ...
          >
            ...
            <p className="text-xs text-muted-foreground truncate">{offer.professional_name}</p>
