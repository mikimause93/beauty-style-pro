  ...
  const loadReferralData = async () => {
    ...
    if (codes && codes.length > 0) {
      ...
      const { data: newCode } = await supabase
        .from("referral_codes")
        .insert({ user_id: user.id, code, reward_qr_coin: 20 })
        .select()
        .single();
    ...
    if (refs) {
      setReferrals(refs);
      const earned = refs.filter(r => r.reward_claimed).length * 20;
      setStats({ totalReferred: refs.length, totalEarned: earned });
    }
  ...
  const shareCode = async () => {
    if (navigator.share) {
      await navigator.share({
        title: "Join STYLE Beauty!",
        text: `Join STYLE Beauty with my code ${referralCode} and get 20 QR Coin bonus! 🎁`,
    ...
    return (
      <MobileLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center">
          ...
          <p className="text-muted-foreground mb-4">Sign in to access the referral program</p>
    ...
    <MobileLayout>
      ...
      <header className="sticky top-0 z-40 glass px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-display font-bold">Invite & Earn</h1>
        </div>
      </header>
      ...
      <div className="p-4 space-y-5">
        ...
        <div className="rounded-2xl gradient-primary p-6 text-center">
          ...
          <p className="text-primary-foreground/80 text-sm">
            For each friend you invite to STYLE Beauty!
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-card border border-border p-5">
          ...
          <div className="flex items-center gap-3 bg-muted rounded-xl p-4 border-2 border-dashed border-primary/30 mb-4">
            <span className="flex-1 text-2xl font-display font-bold tracking-[0.3em] text-center">
              {referralCode || "..."}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={copyCode}
              className="flex items-center justify-center gap-2 py-3 rounded-xl bg-muted hover:bg-muted/80 transition-colors font-semibold text-sm"
            >
            ...
            <button
              onClick={shareCode}
              className="flex items-center justify-center gap-2 py-3 rounded-xl gradient-primary text-primary-foreground font-semibold text-sm"
            >
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="rounded-xl bg-card border border-border p-5 text-center">
            ...
            <p className="text-xs text-muted-foreground">QRC Earned</p>
          </div>
        </div>
        ...
        <div>
          ...
          {referrals.length === 0 ? (
            <div className="text-center py-10 rounded-xl bg-card">
              ...
              <p className="text-muted-foreground text-sm">No referrals yet. Start inviting friends!</p>
            </div>
          ) : (
              ...
              {referrals.map((ref) => (
                <div key={ref.id} className="flex items-center gap-3 p-3 rounded-xl bg-card shadow-card">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold">Friend #{ref.referred_id.slice(0, 6)}</p>
                  ...
                  <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                    ...
                    {ref.reward_claimed ? "✓ +20 QRC" : "Pending"}
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-xl bg-card p-5">
          ...
          <div className="space-y-3">
            {[
              { icon: "📤", text: "Share your unique referral code" },
              { icon: "👤", text: "Your friend signs up with your code" },
              { icon: "🎁", text: "You both earn 20 QR Coins!" },
            ].map((step, i) => (
